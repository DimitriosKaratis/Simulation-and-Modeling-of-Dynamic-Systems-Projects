...
%% Exercise 1 - c
% Lyapunov Method / Series-Parallel Configuration Estimator with Noise
function xxdot = LyapSPNoise(t,xx)

    global A B c d f gamma thetam h0 f0
    ...
    xxdot(4) = a21_est*(x1+h) + a22_est*x2 + b2_est*u + thetam(3) * e1 + thetam(4) * e2; % x2_hat_dot
    xxdot(5) = gamma(1) * e2 * (x1 + h);                                       % a21_hat_dot
